...
namespace Clustering
{
    public class Kmedoids
    {
        ...
        public static List<Centroid> PrepareDocumentCluster(List<DocVector> documentCollection, ref int _counter, int k = 5)
        {
            globalCounter = 0;
            ...
            {
                c = new Centroid();
                c.GroupedDocument = new List<DocVector>();
            ...
            do
            {
                ...
                foreach (DocVector obj in documentCollection)
                {
        ...
        private static Boolean CheckStoppingCriteria(List<Centroid> prevClusterCenter, List<Centroid> newClusterCenter)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    {
                        for (int j = 0; j < newClusterCenter[index].GroupedDocument[0].Tf_idf.Count(); j++)
                        {
                            //
                            if (newClusterCenter[index].GroupedDocument[0].Tf_idf[j] == prevClusterCenter[index].GroupedDocument[0].Tf_idf[j])
                            {
                                count++;
                            }
                        ...
                        if (count == newClusterCenter[index].GroupedDocument[0].Tf_idf.Count())
                        {
                            changeIndex[index] = 0;
                        }
                        else
        ...
        private static int FindClosestClusterCenter(List<Centroid> clusterCenter, DocVector obj)
        {
            ...
            for (int i = 0; i < clusterCenter.Count(); i++)
            {
                ...
                similarityMeasure[i] = SimMetrics.cosSim(clusterCenter[i].GroupedDocument[0].Tf_idf, obj.Tf_idf);
        ...
        private static List<Centroid> CalculateMeanPoints(List<Centroid> _clusterCenter)
        {
            ...
            for (int i = 0; i < _clusterCenter.Count(); i++)
            {
                ...
                if (_clusterCenter[i].GroupedDocument.Count() > 0)
                {
                    ...
                    for (int j = 0; j < _clusterCenter[i].GroupedDocument[0].Tf_idf.Count(); j++)
                    {
                        float total = 0;

                        foreach (DocVector vSpace in _clusterCenter[i].GroupedDocument)
                        {

                            total += vSpace.Tf_idf[j];

                        }

                        //reassign new calculated mean on each cluster center, It indicates the reposition of centroid
                        _clusterCenter[i].GroupedDocument[0].Tf_idf[j] = total / _clusterCenter[i].GroupedDocument.Count();
